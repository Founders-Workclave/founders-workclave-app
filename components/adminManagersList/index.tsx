...
import { getAgencyManagers } from "@/lib/api/superAdmin/agencyService";
...
const AdminManagersPage: React.FC<AllProductManagersPageProps> = ({
  agencyId,
}) => {
  ...
  useEffect(() => {
    const fetchManagers = async () => {
      try {
        setIsLoading(true);
        setError(null);
        const response = await getAgencyManagers(agencyId);

        const transformedManagers: ProductManager[] = response.managerList.map(
          (manager: ApiManager) => ({
